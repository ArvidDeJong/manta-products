<div class="mb-8 mt-4 flex items-center justify-between">
    <div>
        <flux:heading size="xl">Eigenschap</flux:heading>
    </div>
...
<form wire:submit="save">
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
                ...
                @if ($this->fields['name']['active'])
                    <div>
                        <flux:field>
                            ...
                            <flux:input wire:model.blur="name" placeholder="Bijv. Kleur, Materiaal, Afmeting..."
                ...
                @if ($this->fields['code']['active'])
                    <div>
                        <flux:field>
                            ...
                            <flux:input wire:model.blur="code" placeholder="bijv_kleur_materiaal_afmeting"
                ...
                @if ($this->fields['type']['active'])
                    <div>
                        <flux:field>
                            ...
                            <flux:select wire:model.live="type" placeholder="Selecteer een type..."
                ...
                @if ($this->fields['sort']['active'])
                    <div>
                        <flux:field>
                            ...
                            <flux:input type="number" wire:model="sort" placeholder="100" min="0"
                    ...
                    @if ($type === 'select' || $type === 'multiselect')
                        <div>
                            <flux:field>
                                ...
                                <flux:textarea wire:model="configJson" rows="8"
            ...
            <!-- Attribute waarden (alleen bij update) -->
            @if(isset($item) && $item->id)
                <flux:card class="mt-6 space-y-6">
                    <flux:heading size="lg">Attribute waarden</flux:heading>
                    
                    <!-- Bestaande waarden -->
                    @if($item->values && $item->values->count() > 0)
                        <div class="space-y-2">
                            @foreach($item->values->sortBy('sort') as $value)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    @if($editingValueId === $value->id)
                                        <!-- Edit mode -->
                                        <div class="flex-1 grid grid-cols-1 md:grid-cols-4 gap-2">
                                            <flux:input wire:model="editingValueCode" placeholder="Code" />
                                            <flux:input wire:model="editingValueValue" placeholder="Waarde" />
                                            <flux:input type="color" wire:model="editingValueHex" />
                                            <flux:input type="number" wire:model="editingValueSort" placeholder="Sort" />
                                        </div>
                                        <div class="flex gap-2 ml-2">
                                            <flux:button wire:click="updateAttributeValue" variant="primary" size="sm" icon="check">
                                                Opslaan
                                            </flux:button>
                                            <flux:button wire:click="cancelEditValue" variant="ghost" size="sm" icon="x-mark">
                                                Annuleren
                                            </flux:button>
                                        </div>
                                    @else
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2">
                                                @if($value->hex)
                                                    <div class="w-4 h-4 rounded border" style="background-color: {{ $value->hex }}"></div>
                                                @endif
                                                <div>
                                                    <div class="font-medium">{{ $value->value }}</div>
                                                    <div class="text-sm text-gray-600">{{ $value->code }}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <flux:badge variant="outline">Sort: {{ $value->sort }}</flux:badge>
                                            <flux:button wire:click="editAttributeValue({{ $value->id }})" variant="ghost" size="sm" icon="pencil">
                                                Bewerken
                                            </flux:button>
                                            <flux:button wire:click="deleteAttributeValue({{ $value->id }})" 
                                                        wire:confirm="Weet je zeker dat je deze waarde wilt verwijderen?"
                    ...
                    @else
                        <div class="text-center py-8 text-gray-500">
                            Nog geen waarden toegevoegd
                        </div>
                    @endif

                    <!-- Nieuwe waarde toevoegen -->
                    <div class="border-t pt-4">
                        <flux:heading size="md" class="mb-4">Nieuwe waarde toevoegen</flux:heading>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <flux:field>
                                <flux:label>Code</flux:label>
                                <flux:input wire:model="newValueCode" placeholder="bijv_rood" />
                                <flux:error name="newValueCode" />
                            </flux:field>
                            <flux:field>
                                <flux:label>Waarde</flux:label>
                                <flux:input wire:model.blur="newValueValue" placeholder="Rood" />
                                <flux:error name="newValueValue" />
                            </flux:field>
                            <flux:field>
                                <flux:label>Hex kleur (optioneel)</flux:label>
                                <flux:input type="color" wire:model="newValueHex" />
                                <flux:error name="newValueHex" />
                            </flux:field>
                            <flux:field>
                                <flux:label>Sortering</flux:label>
                                <flux:input type="number" wire:model="newValueSort" placeholder="0" />
                                <flux:error name="newValueSort" />
                            </flux:field>
                        </div>
                        <div class="mt-4">
                            <flux:button wire:click="addAttributeValue" variant="primary" icon="plus">
                                Waarde toevoegen
                            </flux:button>
                        </div>
                    </div>
            ...
            @if ($type)
                <flux:card class="space-y-4">
                    ...
                    @elseif($type === 'number')
                        ...
                        @if ($configUnit)
                            <div class="mt-1 text-xs text-gray-500">Eenheid: {{ $configUnit }}</div>
                        @endif
                    @elseif($type === 'textarea')
                        <flux:textarea placeholder="{{ $configPlaceholder ?: 'Tekst invoer...' }}" disabled
                            rows="3" />
                    @elseif($type === 'select')
